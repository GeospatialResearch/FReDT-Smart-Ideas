...
class BasePrecipitationFloodModelGenerator(ABC):
    """This class is to generate precipitation for a flood model"""
    ...
    @property
    @abstractmethod
    def precipitation_var_name(self) -> str:
        """Abstract attribute for the name of the precipitation variable used in the flood model."""

    @property
    @abstractmethod
    def precipitation_var_units(self) -> str:
        """Abstract attribute for the units of the precipitation variable used in the flood model."""

    @property
    @abstractmethod
    def time_var_units(self) -> str:
        """Abstract attribute for the units used for time in the flood model."""

    @property
    @abstractmethod
    def time_var_scale_factor(self) -> int:
        """Abstract attribute for converting time units for the flood model."""
    ...
    def assign_each_precipitation_timestep(
        ...
        for i, t in tqdm(
            ...
            each_precipitation_timestep = self.combined_precipitation_data.sel(time=t)[self.precipitation_var_name]
    ...
    def precipitation_generator(self) -> None:
        ...
        precipitation_var = outfile_precipitation.createVariable(
            self.precipitation_var_name, 'float32', ('time', 'y', 'x')
        )

        # Add units
        time_var.units = self.time_var_units
        x_var.units = 'm'
        y_var.units = 'm'
        precipitation_var.units = self.precipitation_var_units
        time_var.axis = 'T'
        x_var.axis = 'X'
        y_var.axis = 'Y'

        # Assign values
        time_var[:] = np.arange(outfile_precipitation_time) * self.time_var_scale_factor
