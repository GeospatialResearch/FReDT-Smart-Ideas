...
class PrecipitationFloodModelGenerator():
    ...
    def precipitation_generator(self) -> None:  # pylint: disable=no-member
        ...
        precipitation_var = outfile_precipitation.createVariable(
            'rainfall_depth', 'float32', ('time', 'y', 'x')
        )
        ...
        x_var.units = 'm'
        y_var.units = 'm'
        precipitation_var.units = 'mm'
        time_var.axis = 'T'
        x_var.axis = 'X'
        y_var.axis = 'Y'

        # Assign values
        time_var[:] = np.arange(outfile_precipitation_time)
