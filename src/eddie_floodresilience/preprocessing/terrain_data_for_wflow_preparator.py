...
class TerrainDataWflowPreparator:
    ...
    def __init__(
        ...
        self.ds_hydro_org = rxr.open_rasterio(self.path / f"{self.origin_filename}_dem_for_wflow_coarser_nodeps.tif")
        # Add crs
        self.ds_hydro_org = self.ds_hydro_org.rio.write_crs("EPSG:2193")
        self.gdf_riv_org = gpd.read_file(self.path / f"{self.origin_filename}_streams_d8_area_strahler.shp")
    ...
    def write_out_hydro_dataset(
        ...
        # Here "merit_hydro_index.gpkg" is kept for basic automation
        gdf_basins.to_file(
            os.path.join(
                self.path,
                'merit_hydro_index.gpkg'
            )
        )

    def terrain_data_for_wflow_preparator(self) -> None:
